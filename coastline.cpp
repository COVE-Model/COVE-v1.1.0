...
void Coastline::Initialise(string xyfilename)
{
	...
	cout << "Coastline.Initialise: Initialising Coastline from XY file: " << xyfilename << endl;
	;
	...
	VolumeIn = EmptyVector;
	VolumeOut = EmptyVector;
	MinVolumeChange = EmptyVector;
	...
	// These are default values, to change them, use the "set" functions defined in coastline.hpp in your driver file
	ClosureDepth = 10.;			//NEED TO FIND SOMEWHERE BETTER TO DEFINE THESE
	ShorefaceSlope = 0.02;		//NEED TO FIND SOMEWHERE BETTER TO DEFINE THESE

  //Calculate trend
	double dX = X[NoNodes-1]-X[0];
	double dY = Y[NoNodes-1]-Y[0];
  double SpatialGradient = dY/dX;
	//convert to azimuths
	if (dX == 0 && dY < 0) Trend = 180.;
	else if (dX == 0 && dY > 0) Trend = 0.;
	else if (dX > 0) Trend = (180./M_PI)*(M_PI*0.5 - atan(SpatialGradient));
	else if (dX < 0) Trend = (180./M_PI)*(M_PI*1.5 - atan(SpatialGradient));
...
void Coastline::Initialise(string xyfilename, float StartTime)
{
	...
	/*	Read coastline.XY text file */
	cout << "Coastline.Initialise: Initialising Coastline from XY file: " << xyfilename << " at Time: " << StartTime << endl;
...
void Coastline::Initialise(int NodeSpacing, double CoastLength, double TheTrend, int StartBoundaryInput, int EndBoundaryInput)
{
	...
	//initialise coast
	while (Dist<CoastLength-MeanNodeSpacing)
	{
    if (Dist==0)
		{
			//First node
...
void Coastline::ReadCoast(string InputFileName, double Time)
{
	...
	if (ReadCoastFile.is_open())
	{
		...
		if 			(StartBoundary == 1 && EndBoundary == 1) cout << "Coastline.ReadCoast: Boundaries are periodic" << endl << endl;
		else if 	(StartBoundary == 2 && EndBoundary == 2) cout << "Coastline.ReadCoast: Boundaries are fixed" << endl << endl;
		else if 	(StartBoundary == 3 && EndBoundary == 3) cout << "Coastline.ReadCoast: Boundaries are prescribed" << endl << endl;
		else
		{
...
void Coastline::CalculateMorphology()
{
	...
	************************************************************/
	...
	for (int i=0; i<NoNodes; ++i)
	{
      ...
      if ((Orientation[i] > 270) && (FluxOrientation[i-1] < 90)) e1[i] = Orientation[i]-360.-FluxOrientation[i-1];
      else e1[i] = Orientation[i]-FluxOrientation[i-1];
      e2[i] = FluxOrientation[i] - Orientation[i];
      if (e2[i] < -180) e2[i] += 360;
      if (e1[i] < -180) e1[i] += 360;
      LeftOrientation[i] = Orientation[i]-90-e1[i];
      RightOrientation[i] = Orientation[i]-90+e2[i];

		if (LeftOrientation[i]<0) LeftOrientation[i]+=360;
      if (RightOrientation[i]<0) RightOrientation[i]+=360;
      if (LeftOrientation[i]>360) LeftOrientation[i]-=360;
      if (RightOrientation[i]>360) RightOrientation[i]-=360;
...
void Coastline::GetAlpha()
{
	//Function to determine the angle between the shoreline and offshore wave conditions
	double FluxOrientationLast, FluxOrientationNext;
	double Alpha_0_Last, Alpha_0_This, Alpha_0_Next;
	double Theta_0;
	...
	for (int i=0; i<NoNodes; ++i)
	{
		...
		if (Shadows[i] == 1) Theta_0 = ShadowZoneWaveDirection[i];
		else Theta_0 = OffshoreWaveDirection;
		...
		if ((FluxOrientation[i] < 90) && (FluxOrientationLast > 270)) FluxOrientationLast -= 360;
		else if ((FluxOrientation[i] > 270) && (FluxOrientationLast < 90)) FluxOrientationLast += 360;
		if ((FluxOrientation[i] < 90) && (FluxOrientationNext > 270)) FluxOrientationNext -= 360;
		else if ((FluxOrientation[i] > 270) && (FluxOrientationNext < 90)) FluxOrientationNext += 360;		

		//Check Theta_0 or ShadowZoneWave Direction are in the correct orientation with respect to 0 vs 360
		if (Theta_0 > FluxOrientation[i]+180.) Theta_0 -= 360.;
		else if (Theta_0 < FluxOrientation[i]-180.) Theta_0 += 360.;

		//determine alpha_0 angle between coast and wave crest approach
		if (Shadows[i] == 1)
		{
			if (ShadowZoneWaveDirection[i] <= FluxOrientation[i]) Alpha_0_This = FluxOrientation[i]-ShadowZoneWaveDirection[i]-90.;
			else if (ShadowZoneWaveDirection[i] > FluxOrientation[i]+270.) Alpha_0_This = (FluxOrientation[i]+270.)-ShadowZoneWaveDirection[i];
			else Alpha_0_This = 270.-(ShadowZoneWaveDirection[i]-FluxOrientation[i]);
		}
		else if ((Shadows[i] == 2) && (i > 2))
		{
			if (Theta_0 <= FluxOrientationLast) Alpha_0_This = FluxOrientationLast-Theta_0-90.;
			else if (Theta_0 > FluxOrientation[i]+270.) Alpha_0_This = (FluxOrientationLast+270.)-Theta_0;
			else Alpha_0_This = 270.-(Theta_0-FluxOrientationLast);
		}
		else if ((Shadows[i] == 4) && (i < NoNodes-3))
		{
			//determine alpha_0 angle between coast and wave crest approach
			if (Theta_0 <= FluxOrientationNext) Alpha_0_This = FluxOrientationNext-Theta_0-90.;
			else if (Theta_0 > FluxOrientationNext+270.) Alpha_0_This = (FluxOrientationNext+270.)-Theta_0;
			else Alpha_0_This = 270.-(Theta_0-FluxOrientationNext);
		}
		else
		{
			if (Theta_0 <= FluxOrientationNext) Alpha_0_This = FluxOrientation[i]-Theta_0-90.;
			else if (Theta_0 > FluxOrientation[i]+270.) Alpha_0_This = (FluxOrientation[i]+270.)-Theta_0;
			else Alpha_0_This = 270.-(Theta_0-FluxOrientation[i]);
		}
	  ...
	  else Alpha_0_Last = Alpha_0_This;
	  ...
	  else Alpha_0_Next = Alpha_0_This;
    ...
    else if ((Alpha_0_Last > 0) && (Alpha_0_This > 0))
    {
     	...
     	else if ((Alpha_0_Last > 45.) && (Alpha_0_This < 45.)) Alpha_0_This = 45.;
    ...
    else if ((Alpha_0_Next < 0) && (Alpha_0_This < 0))
    {
    	if ((Alpha_0_Next > -45.) && (Alpha_0_This < -45.)) Alpha_0_This = -45.;
     	else if ((Alpha_0_Next < -45.) && (Alpha_0_This> -45.)) Alpha_0_This = -45.;
    ...
    {
     	if (Theta_0 <= Orientation[i]) Alpha_0_This = FluxOrientation[i-1]-Theta_0-90.;
     	else if (Theta_0 > Orientation[i]+270.) Alpha_0_This = (FluxOrientation[i-1]+270.)-Theta_0;
     	else Alpha_0_This = 270.-(Theta_0-FluxOrientation[i-1]);
    }
...
//	    else Alpha_0[i] = FluxOrientation[i]-ShadowZoneWaveDirection[i]-90.;
//	  }
//	  else if (Shadows[i] == 2)
//	  {
...
//	  else if (Shadows[i] == 3)
//	  {
//	    if (Theta_0 <= FluxOrientation[i]) Alpha_0[i] = FluxOrientation[i]-Theta_0-90.;
...
//	}
  ...
  for (int i=0; i<NoNodes; ++i)
  {
    //if we're in the shadow zone then use shadow zone wave characteristics (H_0 == 0 if no diffraction).
    //Otherwise use offshore wave conditions
	  if (Shadows[i] == 1) H_0 = ShadowZoneWaveHeight[i];
		else H_0 = OffshoreWaveHeight;
	  ...
	  else
	  {
		  while (BreakCondition == 0)
		  {
		    ...
		    C = C_0*tanh((2*M_PI*h)/L);					//Wave speed (m/s) set by water depth, L and C_0
	     	...
	     	Alpha = (180./M_PI)*asin((C/C_0)*sin((M_PI/180.)*Alpha_0[i]));		//update theta
			  Kr = sqrt(cos((M_PI/180.)*Alpha_0[i])/cos((M_PI/180.)*Alpha));		//refraction coefficient
...
//	ShadowsCopy = Zeros; Valgrind says this sort of assignment causes a memory error!?
	...
	for (i=1; i<N; ++i)
	{
		if (i>=NoNodes*2-1) reali = i-2*NoNodes+2;
		else if (i>=NoNodes) reali = i-NoNodes+1;
		else reali = i;
		
		// this bit might need some more logic. 
		// Basically, is the wave coming from behind? In which case the cell has to be in shadow
		if (((ShadowAngle < FluxOrientation[reali]) && (ShadowAngle > FluxOrientation[reali]-180.))
				|| ShadowAngle > FluxOrientation[reali]+180.)
	...
	if (N > NoNodes)
	{
		vector<double> TempVec(ShadowsCopy.begin()+NoNodes-1, ShadowsCopy.begin()+2*NoNodes-1);
		Shadows = TempVec;
	}
	else
	{
	...
	for (i=0; i<NoNodes; ++i)
	{
		...
		else if (Shadows[i] == 3)
		{
			...
			while (j < ShadowStart-1)
			{
				...
				//This angle diff HAS to be less than 180
				if (AngleDiff > 180.) AngleDiff -= 360.;
				
				//if (1.5*AngleDiff < -90.) TempWaveHeight = 0;
				//else
				//TempWaveHeight = OffshoreWaveHeight*0.5*(1-sin((M_PI/180.)*(-AngleDiff)));
				TempWaveHeight = OffshoreWaveHeight*0.5*(90.-fabs(AngleDiff))/90.;

				if (TempWaveHeight < ShadowZoneWaveHeight[j] || ShadowZoneWaveHeight[j] <= 0)
				{
	...
	while (!CoastlineQueue.empty())
	{
		...
		else
		{
			if (CellType[i] == 1 && Dsft<ClosureDepth)
			{
				...
				if (LeftOrientation[i] == 180) LeftOrientation[i] = 179.99;
				if (RightOrientation[i] == 180) RightOrientation[i] = 179.99;
				...
				if (b<i) XYNode.X = X0[i]+(X[0]-X[NoNodes-1]), XYNode.Y = Y0[i]+(Y[0]-Y[NoNodes-1]);
				...
				if (MeshOrientation < 0) MeshOrientation += 360;
			...
			//cell down the coast has been processed
		 	else if (CellType[i] == 3)
		 	{
				...
				else
				{
					...
					//Update mesh orientation as mean orientation of all contributing cells
					//simplified 24/7/15
					if (LeftOrientation[i]-RightOrientation[i] < -180) LeftOrientation[i] += 360.;
...
void Coastline::DetermineBeachWidth(Cliffline MyCliffline)
{
	//If the cliffline is an empty object then set beachwidth to NDV and break
	if (MyCliffline.BeachWidth.size() == 0)
	{
	...
	int StartSearch = 0;
	int EndSearch = 0;
	int NearestCliffIndex;
	int BeachFlag = 0;

	//Parameters for dot product search
	double dX, dY, dX12, dY12, dX13, dY13, DotProduct, t, lastt, DistanceToLine, XLine, YLine, SpatialGradient, Azimuth;

	//Reset beach width
	fill(BeachWidth.begin(),BeachWidth.end(),100);

	//loop through cliffline and compare to coastline to get beachwidth
	for (int i=0; i<NoNodes; ++i)
	{
		if (i == 0) NearestCliffIndex = i;
		if (NearestCliffIndex > MyCliffline.NoNodes-SearchDistance) StartSearch = NearestCliffIndex, EndSearch = MyCliffline.NoNodes-1;
		else StartSearch = NearestCliffIndex, EndSearch = NearestCliffIndex+SearchDistance;
		...
		{
			//Get vector x and y
			dX12 = MyCliffline.X[j+1]-MyCliffline.X[j];
			dY12 = MyCliffline.Y[j+1]-MyCliffline.Y[j];

			//Get 2nd Vector properties
			dX13 = X[i]-MyCliffline.X[j];
			dY13 = Y[i]-MyCliffline.Y[j];

			//Find Dot Product
			DotProduct = dX12*dX13 + dY12*dY13;
			//if (DotProduct == 0) {BeachWidth[i] = 0; break;}
			...
			{
				XLine = MyCliffline.X[j];
				YLine = MyCliffline.Y[j];
			}
			else if (t < 0. || t > 1.) continue;
			else
			{
				//Find point along line
				XLine = MyCliffline.X[j] + t*dX12;
				YLine = MyCliffline.Y[j] + t*dY12;
			}

			//find distance to point
			dX = XLine-X[i];
			dY = YLine-Y[i];
			DistanceToLine = sqrt(dX*dX + dY*dY);
			...
			//Find aximuth to point to check for negative beach width and set to 0
			SpatialGradient = dY/dX;
			//convert to azimuths
			if (dX == 0 && dY < 0) Azimuth = 180.;
			else if (dX == 0 && dY > 0) Azimuth = 0.;
			else if (dX > 0) Azimuth = (180./M_PI)*(M_PI*0.5 - atan(SpatialGradient));
			else if (dX < 0) Azimuth = (180./M_PI)*(M_PI*1.5 - atan(SpatialGradient));

			if ((Azimuth < Orientation[i]) && (Azimuth > Orientation[i]-180.) && (DistanceToLine < 5.))
			{
  ...
  {
    CalculateMorphology();
    DetermineBeachWidth(MyCliffline);
  }
}


void Coastline::CalculateMinVolumeChange()
{
 	//calculates the minimum volume change based on the current beach width
 	double a,b,c;
 	for (int i=0; i<NoNodes; ++i)
	{
	  if (BeachWidth[i] == -9999)
	  {
	  	MinVolumeChange[i] = -9999;
	  	break;
	  }
	  else if (Dsf[i] < ClosureDepth)
	  {
	    //define coefficients
	    a = -ShorefaceSlope*(1./6.)*(tan((M_PI/180.)*e1[i]) + tan((M_PI/180.)*e2[i]));
	    b = -ShorefaceSlope*CellWidth[i]/2.;
	    c = ShorefaceSlope*Area[i];
	    MinVolumeChange[i] = -(a*BeachWidth[i]*BeachWidth[i]*BeachWidth[i] + b*BeachWidth[i]*BeachWidth[i] + c*BeachWidth[i]);
	  }
	  else
	  {
	    a = (ClosureDepth/2.)*tan((M_PI/180.)*e1[i]) + tan((M_PI/180.)*e2[i]);
	    b = ClosureDepth*CellWidth[i] + (ClosureDepth*ClosureDepth/2*ShorefaceSlope)*(tan((M_PI/180.)*e1[i]) + tan((M_PI/180.)*e2[i]));
      MinVolumeChange[i] = -(a*BeachWidth[i]*BeachWidth[i] + b*BeachWidth[i]);
	  }
	}

}
void Coastline::TransportSediment(double &TimeDelta, Wave TheWave)
{
  Cliffline CliffVector;
  TransportSediment(TimeDelta, TheWave, CliffVector);
}
void Coastline::TransportSediment(double &TimeDelta, Wave TheWave, Cliffline CliffVector)
{
	//Declare temporary variables
	//double G, dt;
	double VolChange, Excess; //, AreaChange; //PositionChange[i],
	CoastNode RecieverNode;
	int i; //iterators
	...
	//if ((OffshoreWaveDirection < Trend-170.) || (OffshoreWaveDirection > Trend-10.) ||(OffshoreWaveDirection < 0.)) return;
...
//	}
  ...
  //Calculate Morphology and Build Cells!
  CalculateMorphology();

  //Get beach width
	DetermineBeachWidth(CliffVector);

	//CALCULATE MAX POSSIBLE VOLUME CHANGE
	CalculateMinVolumeChange();
	...
	CalculateFlux(FluxType);

	//Calculate beach area
	//BeachArea = CalculateArea(CliffVector);
...
//		//THIS IS FROM THE RELEASE VERSION
...
//		}
		...
		//{
			...
			//Come back and see if this can simplify boundary conditions from above once things are working correctly!
			//if ((StartBoundary == 2) && (i < 2) && (LongshoreFlux[i] > 0)) LongshoreFlux[i] = 0;
			//else if ((EndBoundary == 2) && (i > NoNodes-3) && (LongshoreFlux[i] < 0)) LongshoreFlux[i] = 0;
...
//CALCULATE VOLUME CHANGE
	for (i=0; i<NoNodes; ++i)
	{
		...
		for (int j=0; j<NoRecievers; ++j)
		{
			...
			//If passing to a cell that is beyond the boundary
			else if ((LongshoreFlux[i] > 0) && (RecieverNode.i > NoNodes-3)) VolChange = -LongshoreFlux[i]*((RecieverNode.ShorefaceDepth-TempDepth)/ClosureDepth)*TimeDelta;
			
			else if ((LongshoreFlux[i] > 0) && (LongshoreFlux[RecieverNode.i-1] < 0)) VolChange = (LongshoreFlux[i]+LongshoreFlux[RecieverNode.i-1])*((RecieverNode.ShorefaceDepth-TempDepth)/ClosureDepth)*TimeDelta;

			//If sediment is moving up coast (negative flux) get flux from reciever
			//Flux is negative but we are recieving sed so hence the negative sign
			else if (LongshoreFlux[i] < 0) VolChange = -LongshoreFlux[RecieverNode.i-1]*((RecieverNode.ShorefaceDepth-TempDepth)/ClosureDepth)*TimeDelta;

			//Otherwise pass sediment to reciever from source node
			else VolChange = -LongshoreFlux[i]*((RecieverNode.ShorefaceDepth-TempDepth)/ClosureDepth)*TimeDelta;

			//Check if there is enough sediment to pass, and if not, put it back
			if (VolChange < 0)
			{
			  VolumeOut[i] += VolChange;
			  VolumeIn[RecieverNode.i] -= VolChange;
			  //temp commented out for testing
			  if (VolumeOut[i] < MinVolumeChange[i])
			  {
			    Excess = VolumeOut[i] - MinVolumeChange[i];
			    VolumeOut[i] -= Excess;
			    VolumeIn[RecieverNode.i] += Excess;
			  }
			}
			else if (VolChange > 0)
			{
				...
				VolumeIn[i] += VolChange;
				VolumeOut[RecieverNode.i] -= VolChange;
			  
				//temp commented out for testing
				if (VolumeOut[RecieverNode.i] < MinVolumeChange[RecieverNode.i])
				{
					Excess = VolumeOut[RecieverNode.i] - MinVolumeChange[RecieverNode.i];
					VolumeOut[RecieverNode.i] -= Excess;
					VolumeIn[i] += Excess;
				}
			}
	...
	for (i=0; i<NoNodes; ++i)
	{
		VolumeChange[i] = VolumeIn[i]+VolumeOut[i];
		if ((i == NoNodes-1) && (StartBoundary == 1 && EndBoundary == 1)) VolumeChange[0] = VolumeChange[i];
		else if ((i < 2) && (StartBoundary == 2)) VolumeChange[i] = 0;
		else if ((i > NoNodes-3) && (EndBoundary == 2)) VolumeChange[i] = 0;
	}
	...
	while (ReduceTimeDelta == 1)
	{
		...
		for (i=0; i<NoNodes; ++i)
		{
			...
			{
				cout << "In " << __func__ << " at line " << __LINE__ << ": Large Position Change, reducing timestep!" << endl;
			...
			else if (PositionChange[i] != PositionChange[i])
			{
				...
				cout << "In " << __func__ << " at line " << __LINE__ << ": Found NaN in Position Change! Debug needed!" << endl;
			...
			//more needed here!
			//else if (PositionChange[i] < -(BeachWidth[i]+MinBeachWidth)))
			else if ((BeachWidth[i] != -9999) && (PositionChange[i] < -(BeachWidth[i])))
			{
			  PositionChange[i] = BeachWidth[i];
      }
		}
  ...
  //Effectively just comment this bit out to run diffusivity only
	for (i=0; i<NoNodes; ++i)
	{
...
//		//update Cell Area
//		//Check this calculation, if position change is +ive, area change should be -ive and vica-versa (i.e. erosion = bigger cell)
//		if (Dsf[i] < ClosureDepth)
//		{
//		  AreaChange = PositionChange[i]*CellWidth[i] + 0.5*PositionChange[i]*PositionChange[i]*(tan((M_PI/180)*e1[i]) + tan((M_PI/180)*e2[i]));
//		  Area[i] -= AreaChange;
//		}
	}
	...
	//Calculate beach area
	//double BeachAreaChange = CalculateArea(CliffVector)-BeachArea;
//	double Volume = 0;
//	double VolumeDiff = 0;
//  double Position = 0;
//
//
//	//Write some outputs for debugging
//	//Get beach width
//	DetermineBeachWidth(CliffVector);
//
//	//Output total volume
//	Volume = CalculateTotalVolume();
//	VolumeDiff = Volume-LastVolume;
//	LastVolume = Volume;

//	//test if output file already exists
//	int FileExists = 0;
//	string VolOut = "Area.txt";
//	ifstream oftest(VolOut.c_str());
//	if (oftest) FileExists = 1;
//	oftest.close();

//	//open the output filestream and write headers
//	ofstream WriteVolOut;
//	if (FileExists == 0)
//	{
//		WriteVolOut.open(VolOut.c_str());
//		if (WriteVolOut.is_open()) WriteVolOut << "OffshoreWaveDirection Area AreaChange" << endl;
//	}
//	WriteVolOut.close();

//	//open output filestream again to  coastline data
//	WriteVolOut.open(VolOut.c_str(), fstream::app|fstream::out);

//	//Check if file exists if not open a new one and write headers
//	if (WriteVolOut.is_open()) WriteVolOut << OffshoreWaveDirection << " " << BeachArea << " " << BeachAreaChange << endl;
//  WriteVolOut.close();
...
void Coastline::CheckRecievers()
{
  int AdjacentFlag, jj;

  //write list of recievers to file
	for (int ii=0; ii<NoNodes-1; ++ii)
	{
	  AdjacentFlag = 0;
	  ...
	  if (AdjacentFlag != 1)
	  {
	    cout << "Cell with no adjacent recievers" << endl;
	  }
	}
}

double Coastline::CalculateArea(Cliffline CliffVector)
{
  vector<double> XX, YY;
  XX = X;
  reverse(XX.begin(),XX.end());
  XX.insert(XX.end(), CliffVector.X.begin(),CliffVector.X.end());
  XX.push_back(XX[0]);
  YY = Y;
  reverse(YY.begin(),YY.end());
  YY.insert(YY.end(),CliffVector.Y.begin(),CliffVector.Y.end());
  YY.push_back(YY[0]);
  ...
  for (int i=0;i<N;++i)
  {
    if (i<N) Sum += XX[i]*YY[i+1] - XX[i+1]*YY[i];
    else Sum += XX[i]*YY[0] - YY[i]*XX[0];
  }
  Area = fabs(Sum/2.);
  return Area;
}

//This is a hack job for now
double Coastline::CalculateTotalVolume()
{
  double Volume = 0;
  for (int i=1; i<NoNodes; ++i)
  {
    Volume += BeachWidth[i]*Area[i];
  }
  return Volume;
}

void Coastline::AddVolume(double Volume, int i)
{
  //update array
  VolumeChange[i] = Volume;
...
void Coastline::CalculateFlux(int FluxType)
{
	/*
...
//		5 = Damgaard-Soulsby (Bedload/Shingle only)?
		6 = Bayram?
		7 = Morfett?
		9 = CERC gravel with porosity term taken from Brown et al. (2016)
	...
	*/
	...
	for (int i=0; i<NoNodes; ++i)
	{
	  ...
	  {
		  //CERC Equation for gravel
		  LongshoreFlux[i] = 86400.*0.054
						  *pow(BreakingWaveHeight[i],2.5)
						  *sin((M_PI/180.0)*BreakingWaveAngle[i])
						  *cos((M_PI/180.0)*BreakingWaveAngle[i]);
	  ...
	  {
	    /*CERC gravel equation with porosity term (Brown et al. 2016), and reduction factor of 18 following Soulsby and Damgaard (2005)
	      2000 = wet gravel density (kg per cubic metre)
	      0.45 = porosity factor for gravel
	      19002.33 = 2^3.5 * sqrt(9.81) * (2000-1025) * (1-0.45)
	     */
	    //LongshoreFlux[i] = 86400.*0.054*(rho_w*pow(BreakingWaveHeight[i],2.5)*sin(2*((M_PI/180)*BreakingWaveAngle[i]))/19002.33);
	    LongshoreFlux[i] = 86400.*0.054*(rho_w*pow(BreakingWaveHeight[i],2.5)*sin(2*((M_PI/180)*BreakingWaveAngle[i]))/(pow(2,3.5)*pow(g,0.5)*(2000-rho_w)*(1-0.45)));
	    //LongshoreFlux[i] = LongshoreFlux[i]/18;
  ...
  //										*(1.-0.4*(BreakingWaveAngle[i]/90.)*(1.-BreakingWaveAngle[i]/90.)) ,2.5);
	  ...
	  {
		  //Damgaard and Soulsby (1997) (BP-2005??) equation for bedload transport aimed at calculating longshore transport for shingle
...
void Coastline::WriteFluxesFile()
{
  //write list of shadows to file
	ofstream WriteFluxesOut;
	WriteFluxesOut.open("Fluxes.txt");
	for (int ii=0; ii<NoNodes; ++ii) WriteFluxesOut << LongshoreFlux[ii] << " " << VolumeChange[ii] << " " << PositionChange[ii] << endl;
	WriteFluxesOut.close();
}

void Coastline::WriteAlpha()
{
  //write list of shadows to file
	ofstream WriteAlphaOut;
	WriteAlphaOut.open("Alpha.txt");
	for (int ii=0; ii<NoNodes; ++ii) WriteAlphaOut << Alpha_0[ii] << endl;
	WriteAlphaOut.close();
}
...
void Coastline::WriteBeachWidth()
{
  //test if output file already exists
  string OutputFileName = "BeachWidth.txt";
	int FileExists = 0;
	ifstream oftest(OutputFileName.c_str());
	if (oftest) FileExists = 1;
	oftest.close();
	...
	{
		//write BeachWidth
		WriteCoastFile << setprecision(4) << OffshoreWaveDirection;
		for (int i=0; i<NoNodes; ++i) WriteCoastFile << setprecision(5) << " " << BeachWidth[i];
		WriteCoastFile << endl;
	}
}

void Coastline::WriteVolumeChange()
{
  //test if output file already exists
  string OutputFileName = "VolumeChange.txt";
	int FileExists = 0;
	ifstream oftest(OutputFileName.c_str());
	if (oftest) FileExists = 1;
	oftest.close();
	...
	if (WriteCoastFile.is_open())
	{
		//write VolumeChange
		WriteCoastFile << setprecision(4) << OffshoreWaveDirection;
		for (int i=0; i<NoNodes; ++i) WriteCoastFile << setprecision(5) << " " << VolumeChange[i];
		WriteCoastFile << endl;
	}
}
#endif
