...
void Coastline::TransportSediment(double &TimeDelta, Wave TheWave, Cliffline CliffVector)
{
	//Declare temporary variables
	//double G, dt;
	double VolChange, Excess, TempDepth; //, AreaChange; //PositionChange[i], 
	CoastNode RecieverNode;
	int i, NoRecievers; //iterators
	...
	if ((OffshoreWaveDirection < Trend-170.) || (OffshoreWaveDirection > Trend-10.)) return;
...
//	}
  ...
  //Get beach width
	...
	for (i=0; i<NoNodes; ++i)
	{
		...
		//OLD STABLE BOUNDARY CONDITIONS ARE OVERLY COMPLICATED!!
...
//			VolumeOut[i] = (Dsf[i]/ClosureDepth)*(LongshoreFlux[i-1]-LongshoreFlux[i])*(1.-LostFluxFraction)*TimeDelta;
//		}
//		
...
//		}
		...
		//{
			...
			if (((StartBoundary == 2) && (i < 2) && (LongshoreFlux[i] > 0))
				|| ((EndBoundary == 2) && (i > NoNodes-3) && (LongshoreFlux[i] < 0)))
			{
				LongshoreFlux[i] = 0;
				VolumeIn[i] = 0;
				VolumeOut[i] = 0;
			...
			else
			{
				...
				for (int j=0; j<NoRecievers; ++j)
				{
					...
					//Check for boundary conditions in reciever
					if ((EndBoundary == 2) && (RecieverNode.i > NoNodes-3) && (LongshoreFlux[RecieverNode.i-1] < 0)) continue;
				
					//need to handle round the back here!!
					if (RecieverNode.i == 0) VolChange = LongshoreFlux[NoNodes-2]*((RecieverNode.ShorefaceDepth-TempDepth)/ClosureDepth)*(1.-LostFluxFraction)*TimeDelta;
				
					//If passing to a cell that is trying to pass back, take the sum of the two fluxes
					if ((LongshoreFlux[i] > 0) && (LongshoreFlux[RecieverNode.i-1] < 0)) VolChange = (LongshoreFlux[i]+LongshoreFlux[RecieverNode.i-1])*((RecieverNode.ShorefaceDepth-TempDepth)/ClosureDepth)*TimeDelta;
	...
	for (i=0; i<NoNodes; ++i)
	{
  	...
  	if ((StartBoundary == 2) && (i < 2)) VolumeChange[i] = 0;
  	else if ((EndBoundary == 2) && (i > NoNodes-3)) VolumeChange[i] = 0;
  	else VolumeChange[i] = VolumeIn[i]+VolumeOut[i];
