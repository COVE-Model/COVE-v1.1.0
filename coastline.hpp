...
class Coastline
{
  friend class Cliffline;
	...
	vector<double> MinVolumeChange; //Maximium possible negative volume change based on beach width
	vector<double> LongshoreFlux; //Volume of sediment transported alongshore (in m3)
	vector<double> VolumeChange; //Volume change in each cell during a particular timestep (m3)
	vector<double> VolumeOut; //Volume leaving (m3)
	vector<double> VolumeIn; //Volume entering (m3)
	vector<double> PositionChange; //Magnitude of change in shoreline position (m)
	vector<double> BeachWidth; //Width of beach to nearest cliff segment (m)
	vector<double> Volume; //Total volume of sediment available for transport (m3)
	vector<double> Area; //Surface area of shoreface cell (m2)
	vector<double> e1; //angle of upcoast cell boundary to cell orientation normal (o)
	vector<double> e2; //angle of downcoast cell boundary to cell orientation normal (o)
	...
	vector<double> X0; //X position of cell boundary tip
	vector<double> Y0; //Y position of cell boundary tip
	vector<double> XL; //X position of cell boundary left edge
	vector<double> YL; //Y position of cell boundary left edge
	vector<double> XR; //X position of cell boundary right edge
	vector<double> YR; //Y position of cell boundary right edge
	vector<double> Dsf; //Shoreface Depth at cell bottom
	vector<double> Alpha_0; //Angle of offshore wave


	vector<int> Fixed; //Is coastline fixed? 1 = yes, 0 = np
	vector<int> NodesToRemove; //Flag for removing nodes that are getting squashed
	...
	int ShadowFlag; //Some flags...
	int TriangleFlag;
	int BuildCellsFlag;
	int NodeAddedFlag;

	double Trend; //mean orientation of the coast, needed for U-A wave climates? may be unused
	double TotalVolume; //For debugging

	//Wave parameters
	double OffshoreWavePeriod, OffshoreWaveHeight, OffshoreWaveDirection;
	double ClosureDepth; //Depth at bottom of shoreface
	double ShorefaceSlope; //Gradient of the shoreface

	int FluxType; //Type of longshore flux, defaults to 1 = CERC
	int RefDiffFlag; //Is ref/diff turned on or not?
	double FluxFraction; //Is some flux provided at the model boundary?
	double LostFluxFraction; //How much of the longshore flux is lot offshore

	double LastVolume;//may be redundant now
  ...
  @date 29/10/2015 */
	...
	/* @brief Calculates the angle between the shoreline and incoming wave direction
	following Ashton and Murray (2006), including dealing with the shadow zone. Different
	discretisation schemes for shoreline angle depending on whether waves are high angle
	and whether we're dealing with shadows.
	@author Martin D. Hurst
  @date 16/12/2015 */
	void GetAlpha();
  ...
  @date 29/10/2015 */
	...
	void CalculateMinVolumeChange();
	double CalculateArea(Cliffline CliffVector);
	void CheckRecievers();
  ...
  void WriteFluxesFile();
  void PrintCoastlineQueue();
  void WriteAlpha();
  void WriteBeachWidth();
  void WriteVolumeChange();
  ...
  public:
	...
	/// @author Martin D. Hurst
	/// @date 28/10/2015
	Coastline(string xyfilename)  : FluxType(1), RefDiffFlag(0), FluxFraction(0.), LostFluxFraction(0.) { Initialise(xyfilename); }
	...
	/// @author Martin D. Hurst
	/// @date 28/10/2015
	Coastline(string xyfilename, float StartTime)  : FluxType(1), RefDiffFlag(0), FluxFraction(0.), LostFluxFraction(0.) { Initialise(xyfilename, StartTime); }
	...
	Coastline(int MeanNodeSpacing, double CoastLength, double Trend, int StartBoundary, int EndBoundary)  : FluxType(1), RefDiffFlag(0), FluxFraction(0.), LostFluxFraction(0.)
	{
  ...
  /// @brief Measure beach width to a cliffline object
	/// @details Function to quantify beach width by measuring shortest distance
	/// to a nearby cliffline vector object.
	/// @author Martin D. Hurst
	/// @date 13/1/2016
  void DetermineBeachWidth(Cliffline MyCliffline);
  ...
  @author Martin D. Hurst
	@date 29/10/2015
	*/
	void TransportSediment(double &TimeDelta, Wave TheWave);
	void TransportSediment(double &TimeDelta, Wave TheWave, Cliffline CliffVector);
  ...
  /// @author Martin D. Hurst
  /// @date 29/10/2015
	void CalculateFlux(int FluxType);
  ...
  /// @date 29/10/2015
	...
	/// @brief Add volume to a coastline cell
	/// @details Adds a particular volume to a coastline cell and
	/// determines the associated position change in the coastline
	/// before updating the coastline vector
	/// @param Volume double, the volume to add
	/// @param Index int, the index where the volume is to be added
	/// @author Martin D. Hurst
  /// @date 13/1/2016
	void AddVolume(double Volume, int i);

	/*****************************************\
	| Set Functions to assign private members |
	\*****************************************/

	/* @brief Set Sedimet transport flag
	@details Function to assign the type of sediment transport, 1 = CERC
	@param MyCERCFlag int, Flag for flux equation
	@author Martin D. Hurst
	@date 14/1/2016 */
	void SetFluxType(int NewFluxType) {FluxType = NewFluxType;}

	/* @brief Set Diffraction Flag
	@details Function to set the flag for whether to allow diffraction
	of waves into shadowed regions of the coastline/cliffline model
	@param Flag int, 1 = Yes, 0 = no ref/diff
	@author Martin D. Hurst
	@date 14/1/2016 */
	void SetRefDiffFlag(int NewFlag) {RefDiffFlag = NewFlag;}

	/* @brief Set the efficiency in removing sediment of boundary conditions
	@details This needs some testing
	@param FluxFraction double, the fraction of sediment removed
	@author Martin D. Hurst
	@date 14/1/2016 */
	void SetFluxFraction(double NewFluxFraction)	{FluxFraction = NewFluxFraction;}

	/* @brief Set the fraction of sediment flux that is assumed to be lost offshore
	@details This needs some testing
	@param LostFluxFraction double, the fraction of sediment removed
	@author Martin D. Hurst
	@date 14/1/2016 */
  void SetLostFluxFraction(double NewLostFluxFraction)	{LostFluxFraction = NewLostFluxFraction;};

  /* @brief Set the depth at the bottom of the shoreface
	@details Sets the shoreface depth, default is 10m
	@param ClosureDepth double, the depthh of the shoreface
	@author Martin D. Hurst
	@date 07/07/2016 */
  void SetShorefaceDepth(double NewClosureDepth)	{ClosureDepth = NewClosureDepth;};

  /* @brief Set the shoreface slope
	@details Sets the shoreface slope, default is 0.02
	@param ShorefaceSlope double, the slope of shoreface
	@author Martin D. Hurst
	@date 07/07/2016 */
  void SetShorefaceSlope(double NewShorefaceSlope)	{ShorefaceSlope = NewShorefaceSlope;};
