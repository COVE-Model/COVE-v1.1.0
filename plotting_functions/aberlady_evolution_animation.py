...
CliffFile = "../driver_files/Aberlady_Cliffline_Output.xy"
CoastFile = "../driver_files/Aberlady_Coastline_Output.xy"
...
xmin = np.min(InitX)-100
xmax = np.max(InitX)+100
ymin = np.min(InitY)-100
ymax = np.max(InitY)+100
# xmin = 652000
...
ID, Fixed = np.loadtxt("../driver_files/Aberlady_Fixed_Cliffs.data",dtype='int32',skiprows=1,unpack=True)
...
#            plt.plot(InitX[ii:ii+2],InitY[ii:ii+2],'k-',lw=1.5)
    ...
    plt.plot(CoastX,CoastY, 'b.-', lw=0.5)
